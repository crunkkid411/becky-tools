...
// liveFreeModels asks the endpoint which :free models ACTUALLY exist right now.
// Cached for the process.
//
// WHY THIS IS NOT A HARDCODED LIST: I hand-wrote a rotation containing
// "minimax/minimax-m3:free" and "z-ai/glm-5.2:free". NEITHER EXISTS on
// OpenRouter — both are free only via the local router — so two of five
// fallbacks were guaranteed 404s. Guessing a model name is the same failure as
// writing a rule in prose: it depends on somebody remembering correctly. The
// catalogue is the source of truth, so a fabricated id cannot survive.
var (
	liveFreeOnce  bool
	liveFreeCache map[string]bool
)

func liveFreeModels(key string) map[string]bool {
	if liveFreeOnce {
		return liveFreeCache
	}
	liveFreeOnce = true
	liveFreeCache = map[string]bool{}
	...
	if err != nil {
		return liveFreeCache // offline: fall through to the static order
	}
...
// failure. Entries the endpoint does not actually offer are DROPPED, so a
// fallback is never spent on a name that cannot work.
func rotationFor(first, key string) []string {
	live := liveFreeModels(key)
	keep := func(id string) bool { return len(live) == 0 || live[id] }

	out := []string{first} // always honour the explicit choice, even if unlisted
	...
	// Anything else free the endpoint offers, so the chain survives the
	// hardcoded names going stale.
	for id := range live {
		if !seen[id] {
			out = append(out, id)
			seen[id] = true
...
func openRouterModel(name string, verbose bool) subs.ModelFunc {
	...
	return func(ctx context.Context, prompt string) (string, error) {
		...
		var lastErr error
		for _, m := range rotationFor(model, key) {
