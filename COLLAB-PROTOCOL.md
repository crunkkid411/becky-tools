...
| becky-scout (`SPEC-SCOUT.md`) | cloud 2026-06-16 | BUILT (deterministic core + tests + CLI + "useful to you" lane + `--from-json`; yt-dlp fetch + optional model assessor stubbed) | `cmd/scout` + `internal/scout` | YouTube playlist → becky improve/extend findings + personally-useful suggestions (`interests.go`); corroborate-then-conclude over freshness manifest + capability catalog; sibling of becky-radar. Demoed on Jordan's real "ai useful" playlist via `--from-json` (ytInitialData scrape, no yt-dlp). Left for local: wire `PlaylistSource` via yt-dlp + optional `Assessor` via local llama.cpp |
