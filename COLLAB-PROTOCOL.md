...
| Short-form repurposing: moment selection (`cmd/becky-moment` + `internal/moment`) | cloud 2026-08-15 | **BUILT + PROVEN** (`--selftest` 13/13; real CLI run) | `cmd/becky-moment`, `internal/moment` | Structure measured deterministically, CONTENT judged by LLM, `Rank` corroborates (agree→conclusion, disagree→held at the LOWER signal, one signal→candidate). Pause threshold DERIVED (`AutoThoughtGap` p75), not a constant — the Parakeet lesson at cue scale. Judge = **OpenCode Zen** (Jordan's key, authorised 2026-08-15) with 3 spending guards; resold Claude hard-blocked even with `--allow-paid`. Emits becky-hits' shape; **seam test parses `cmd/becky-hits/main.go`** so a rename there fails here. LEFT: verify the Zen free-model id on one real run (HANDOFF §7 step D) |
| Short-form repurposing: face tracking (`internal/facetrack`) | cloud 2026-08-15 | **BUILT + TESTED — deliberately NOT wired** | `internal/facetrack` | Persistent track IDs: IoU + ArcFace-embedding rescue, deterministic tie-breaks, `CoverageIn` measures presence instead of asserting it. Fixed a real identity-merge bug (stale tracks retired AFTER association could win a detection 40 frames later). **BLOCKED ON THE MODEL BOUNDARY:** `face_embed.py` returns only the most prominent face per image; a tracker needs all of them. Left unconnected ON PURPOSE rather than shimmed to one face — HANDOFF §7 step E has the work order |
| Short-form repurposing: steps 3-6 (LR-ASD, crop path, render) | unclaimed | NOT STARTED — needs the native CV dep | planned | Jordan ratified **MediaPipe + OpenCV/gocv are IN** (2026-08-15). Do NOT approximate these in pure Go — that is the documented failure pattern (HANDOFF §2.3, research §6). HANDOFF §7 step F is the dependency spike |
