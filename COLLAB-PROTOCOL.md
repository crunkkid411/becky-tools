...
| becky-scout (`SPEC-SCOUT.md`) | cloud 2026-06-16 | BUILT (deterministic core + CLI; yt-dlp + assessor stubbed) | `cmd/scout` + `internal/scout` | YouTube playlist → becky improvement radar; 3-signal corroboration (dep-match, capability, assessor); useful-to-Jordan lane; --from-json offline mode; Left for local: wire ytdlpSource (yt-dlp --flat-playlist) + optional llamaAssessor (Qwen3-4B) per SPEC-SCOUT.md wiring contracts |
