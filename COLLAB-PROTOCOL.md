...
| Short-form repurposing pipeline (`research/shorts-models.md` + `HANDOFF-SHORTS-PIPELINE.md`) | cloud 2026-08-15 | **RESEARCH ONLY — nothing built, awaiting Jordan's go/no-go** | docs only; planned `cmd/becky-moment`, face tracking in `internal/faceembed`, LR-ASD pyhelper | Model comparison done + verdicts (becky better at 3 stages, absent at 5). ONE new model recommended: **LR-ASD**. Blocking decisions in HANDOFF §6 — chiefly **native CV dep (gocv/MediaPipe) yes-or-no**, which gates steps 4-6. Step 0 (`becky-reel` +1.27 s fps drift) is a prerequisite fix and is independently useful — **claim it separately if you take it**. Do NOT build steps 3-4 as a pure-Go approximation; see HANDOFF §2.3 |
